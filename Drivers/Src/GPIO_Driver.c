 ...
 * @brief Fichier source du driver GPIO
 *********************************************************************/
...
//by software and the returned value is always 0x0.e"
	...
	*Ctrl &= ~(0x0F << PORTCONF_SIZE*registre);
...
{
	GPIO->BSRR = (0x01 << GPIO_Pin);//pas besoin de |= car cela est plus efficace et c'est une bascule SET:RESET
	return MyGPIO_Read(GPIO, GPIO_Pin);
}
int MyGPIO_Reset(GPIO_TypeDef * GPIO, char GPIO_Pin)
{
	GPIO->BSRR = (0x01 << (GPIO_Pin + 16)); //pas besoin de |= car cela est plus efficace et c'est une bascule SET:RESET
	return MyGPIO_Read(GPIO, GPIO_Pin);
}
